...
// TestSectionSelectStepTrimsToSectionCuts verifies that only the selected section's cuts
...
func TestSectionSelectStepTrimsToSectionCuts(t *testing.T) {
	sectionIndex := 1
	sc := &Context{
		Task: &domain.Task{
			JobID:        "short-1",
	...
	if err := (SectionSelectStep{}).Execute(context.Background(), sc); err != nil {
		t.Fatalf("Execute() error = %v", err)
	}

	if len(sc.VideoRecipe.Cuts) != 2 {
		t.Fatalf("cuts = %d, want 2 (10s cut split into 8s + 4s)", len(sc.VideoRecipe.Cuts))
	}
	...
	if got := sc.VideoRecipe.Cuts[0].StartSec; got != 10 {
		t.Errorf("cut[0] start = %v, want 10", got)
	}
	if got := sc.VideoRecipe.Cuts[1].StartSec; got != 18 {
...
// TestSectionSelectStepRejectsOutOfRangeSection verifies out-of-range section indexes fail.
func TestSectionSelectStepRejectsOutOfRangeSection(t *testing.T) {
	sectionIndex := 5
	sc := &Context{
		Task: &domain.Task{
			JobID:        "short-1",
	...
	err := (SectionSelectStep{}).Execute(context.Background(), sc)
	if err == nil || !strings.Contains(err.Error(), "out of range") {
		t.Fatalf("Execute() error = %v, want out of range error", err)
	}
}

// TestSectionSelectStepRejectsEmptySection verifies sections without cuts fail rather than
// enqueueing an empty video generation.
func TestSectionSelectStepRejectsEmptySection(t *testing.T) {
	sectionIndex := 1
