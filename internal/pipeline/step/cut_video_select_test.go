...
func runCutVideoSelect(t *testing.T, recipe *video.Recipe, cutIndex int, usePreviousVideo bool) error {
	t.Helper()
	return CutVideoSelectStep{UsePreviousVideo: usePreviousVideo}.Execute(context.Background(), &Context{
		Task: &domain.Task{
			JobID:     "mv-2",
...
// TestCutVideoSelectResetsRestOfChain is the point of the step: in a video-to-video chain, a
...
// TestCutVideoSelectKeepsAllCutsAndKeyframes pins two things the step must not do: drop the
