...
// TestRecipeLoadStepLoadsRecipeURLAndAppliesAudioURL verifies recipe loading and task audio URL application.
func TestRecipeLoadStepLoadsRecipeURLAndAppliesAudioURL(t *testing.T) {
	...
	if err := (RecipeLoadStep{}).Execute(context.Background(), sc); err != nil {
		t.Fatalf("Execute() error = %v", err)
	}
	if sc.Recipe == nil {
		t.Fatal("recipe was not loaded")
	}
	if sc.VideoRecipe == nil || len(sc.VideoRecipe.Cuts) == 0 {
		t.Fatal("video recipe cuts were not built")
	}
	if got := sc.VideoRecipe.Cuts[0].AudioReference; got != "gs://bucket/music.mp3" {
		t.Fatalf("AudioReference = %q, want audio URL", got)
	}
}

// TestRecipeLoadStepLoadsVideoRecipeURL verifies keyframe VideoRecipe loading from GCS.
func TestRecipeLoadStepLoadsVideoRecipeURL(t *testing.T) {
	reader := staticReader{content: `{
	...
	if sc.VideoRecipe == nil || len(sc.VideoRecipe.Cuts) != 1 {
		t.Fatalf("video recipe was not loaded: %#v", sc.VideoRecipe)
	}
	if got := sc.VideoRecipe.Cuts[0].KeyframeReference; got != "gs://bucket/keyframe.png" {
...
// TestRecipeLoadStepAbsolutizesRelativeKeyframes pins that job-relative keyframe paths from a
...
func TestRecipeLoadStepAbsolutizesRelativeKeyframes(t *testing.T) {
	reader := staticReader{content: `{
		"title": "legacy recipe",
		"cuts": [
	...
	for i, expected := range want {
		if got := sc.VideoRecipe.Cuts[i].KeyframeReference; got != expected {
			t.Errorf("cut[%d].KeyframeReference = %q, want %q", i, got, expected)
		}
	}
}

// TestCutKeyframeStepAppliesTaskCharacterID verifies that task character IDs are applied during keyframe generation.
func TestCutKeyframeStepAppliesTaskCharacterID(t *testing.T) {
	sc := &Context{
		Task: &domain.Task{
			JobID:       "job-1",
			Command:     domain.CommandMVFromKeyframeVideoRecipe,
	...
	if err := (CutKeyframeStep{}).Execute(context.Background(), sc); err != nil {
		t.Fatalf("Execute() error = %v", err)
	}
	if sc.VideoRecipe == nil || len(sc.VideoRecipe.Cuts) == 0 {
		t.Fatal("video recipe cuts were not built")
	}
	if got := sc.VideoRecipe.Cuts[0].CharacterID; got != "zundamon" {
		t.Fatalf("CharacterID = %q, want zundamon", got)
	}
	if sc.Recipe == nil || len(sc.Recipe.Sections) == 0 {
