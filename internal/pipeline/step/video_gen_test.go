...
// TestVideoGenerationStepEnqueuesContinuationAfterOneCut verifies continuation task enqueueing after one cut.
func TestVideoGenerationStepEnqueuesContinuationAfterOneCut(t *testing.T) {
	recipe := &video.Recipe{
...
// TestVideoGenerationStepPropagatesContinuationEnqueueFailure verifies that a genuine
...
func TestVideoGenerationStepPropagatesContinuationEnqueueFailure(t *testing.T) {
	recipe := &video.Recipe{
		MusicRecipe: video.MusicRecipe{Title: "test"},
		Cuts: []video.Cut{
			{CutIndex: 1, VisualAnchor: "first", DurationSec: 8},
...
// TestVideoGenerationStepPrefersDirectRunnerWhenWorkflowExists verifies the production
// path keeps using per-cut continuation even when orchestrator workflows are configured.
func TestVideoGenerationStepPrefersDirectRunnerWhenWorkflowExists(t *testing.T) {
	recipe := &video.Recipe{
		MusicRecipe: video.MusicRecipe{Title: "test"},
		Cuts: []video.Cut{
			{CutIndex: 1, VisualAnchor: "first", DurationSec: 8},
...
// TestVideoGenerationStepContinuationFromShortSectionUsesContinuationCommand verifies that
...
func TestVideoGenerationStepContinuationFromShortSectionUsesContinuationCommand(t *testing.T) {
	sectionIndex := 0
	recipe := &video.Recipe{
		MusicRecipe: video.MusicRecipe{Title: "test"},
		Cuts: []video.Cut{
...
// TestVideoGenerationStepAddsOutputPathToContext verifies that video generation receives the output path through context.
func TestVideoGenerationStepAddsOutputPathToContext(t *testing.T) {
	recipe := &video.Recipe{
		MusicRecipe: video.MusicRecipe{Title: "test"},
		Cuts: []video.Cut{
			{CutIndex: 1, VisualAnchor: "first", DurationSec: 8},
...
// TestVideoGenerationStepExpandsUnsupportedDurations verifies that cuts longer than Veo's
// supported durations are split into sub-cuts before generation in the full MV flow.
func TestVideoGenerationStepExpandsUnsupportedDurations(t *testing.T) {
	recipe := &video.Recipe{
		MusicRecipe: video.MusicRecipe{Title: "test"},
		Cuts: []video.Cut{
			{
...
// that this step deliberately no longer reads, since honoring it caused keyframe generation
...
// TestVideoGenerationStepForcesEightSecondsForReferenceToVideo reproduces job
...
func TestVideoGenerationStepForcesEightSecondsForReferenceToVideo(t *testing.T) {
	characters, err := characterkit.NewCharacters([]characterkit.Character{
		{
			ID:           "tsumugi",
			Name:         "Tsumugi",
	...
	if err := st.Execute(context.Background(), &Context{
		Task:        task,
		VideoRecipe: recipe,
		Characters:  characters,
	}); err != nil {
	...
	flt2 := VideoGenerationStep{Runner: runner2}
