...
func TestSceneSplitStepExpandsLongCutsBeforeKeyframeGeneration(t *testing.T) {
	recipe := &video.Recipe{
		ProjectTitle: "test",
	...
	err := (SceneSplitStep{}).Execute(context.Background(), &Context{
...
func TestSceneSplitStepAllocatesVideoToVideoChainBlocks(t *testing.T) {
	recipe := &video.Recipe{
		ProjectTitle: "test",
	...
	err := (SceneSplitStep{UsePreviousVideo: true}).Execute(context.Background(), &Context{
		Task:        &domain.Task{JobID: "job-1", Command: domain.CommandVideoRecipeCreate},
		VideoRecipe: recipe,
	})
	if err != nil {
...
// TestSceneSplitStepVideoToVideoTracksMusicTimeline reproduces the real-job bug where two
...
func TestSceneSplitStepVideoToVideoTracksMusicTimeline(t *testing.T) {
	recipe := &video.Recipe{
		ProjectTitle: "test",
		MusicRecipe:  video.MusicRecipe{Title: "test"},
		Cuts: []video.Cut{
...
// TestSceneSplitStepKeyframeScenesDiffusesRoundingError verifies the non-chain (keyframe)
...
func TestSceneSplitStepKeyframeScenesDiffusesRoundingError(t *testing.T) {
	recipe := &video.Recipe{
		ProjectTitle: "test",
		MusicRecipe:  video.MusicRecipe{Title: "test"},
		Cuts: []video.Cut{
...
// TestSceneSplitStepIsIdempotent pins the property the draft flow depends on: a recipe that
// already went through scene splitting must survive a second pass unchanged. Both
// mv_from_keyframe_video_recipe (RecipeLoad -> SceneSplit) and the draft flow re-run the step
// over an already-split recipe, so a non-idempotent pass would silently re-plan the cuts the user
// just reviewed.
func TestSceneSplitStepIsIdempotent(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			st := SceneSplitStep{UsePreviousVideo: tc.usePreviousVideo}
			...
			if err := st.Execute(context.Background(), newContext()); err != nil {
...
// TestSceneSplitStepKeepsKeyframesOnOneToOneReallocation pins the half of the keyframe-reuse
// fix that lives here: a cut that re-allocates to a single block keeps the image already baked
// for it, so CutKeyframeStep can skip regeneration. Without this, "generate a video from the
// saved keyframes" re-bakes every image first.
func TestSceneSplitStepKeepsKeyframesOnOneToOneReallocation(t *testing.T) {
	for _, tc := range []struct {
		name             string
		usePreviousVideo bool
	}{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := (SceneSplitStep{UsePreviousVideo: tc.usePreviousVideo}).Execute(context.Background(), &Context{
...
// TestSceneSplitStepDropsKeyframeWhenCutIsResplit pins the conservative half: once a cut is
// divided, one image can no longer stand for several cuts that each get their own scene beat.
func TestSceneSplitStepDropsKeyframeWhenCutIsResplit(t *testing.T) {
	recipe := &video.Recipe{
		ProjectTitle: "test",
		MusicRecipe:  video.MusicRecipe{Title: "test"},
		Cuts: []video.Cut{{
