...
// historyCreatedAtRaw is passed to paging.WithSortKey, so a regression here
// silently reorders the history list rather than failing outright.
...
func TestHistorySortKeyOrdersByCreatedAtDesc(t *testing.T) {
	...
	selected, meta := paging.SelectIDs(jobIDs, 1, 10, paging.WithSortKey(historyCreatedAtRaw))
