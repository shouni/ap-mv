...
func TestExpandCutsToSupportedDurationsResumedGenerationDoesNotCascadeReset(t *testing.T) {
	...
	// matching Veo's real ~30s API ceiling; color drift from repeated continuations is
	// handled separately by colorCorrectExtensionCut rather than by resetting more often
	// (frequent resets make image_to_video/reference_to_video's choppier motion more common).
