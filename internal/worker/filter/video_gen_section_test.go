...
// cut.SectionIndex は1始まりです（Normalize / SectionSelectFilter と同じ規則）。
...
func TestSectionScopedGenerationKeepsOtherSectionsInTheRecipe(t *testing.T) {
	...
	queue := &captureQueue{}
	flt := VideoGenerationFilter{Runner: sequenceRunner{}, SectionScoped: true}

	// セクション 2（0始まり index 1）だけを生成する。
	err := flt.Execute(context.Background(), sectionScopedContext(recipe, 1, queue))
...
func TestSectionScopedGenerationStopsWhenTheSectionIsDone(t *testing.T) {
	...
	if err := flt.Execute(context.Background(), sectionScopedContext(recipe, 1, queue)); err != nil {
...
func TestSectionScopedGenerationMarksChainStartAfterASkippedGap(t *testing.T) {
	recipe := sectionScopedRecipe()
	queue := &captureQueue{}
	flt := VideoGenerationFilter{Runner: sequenceRunner{}, SectionScoped: true, UsePreviousVideo: true}
...
func TestSectionScopedGenerationRejectsAnEmptySection(t *testing.T) {
	...
	if err := flt.Execute(context.Background(), sectionScopedContext(recipe, 1, &captureQueue{})); err == nil {
...
func TestSectionScopedGenerationRejectsAnOutOfRangeSection(t *testing.T) {
	...
	if err := flt.Execute(context.Background(), sectionScopedContext(sectionScopedRecipe(), 9, &captureQueue{})); err == nil {
...
func TestContinuationRemembersTheOriginCommand(t *testing.T) {
	...
	if err := flt.Execute(context.Background(), sectionScopedContext(recipe, 1, queue)); !errors.Is(err, ErrPipelineDeferred) {
