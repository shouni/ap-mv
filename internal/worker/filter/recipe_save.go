...
// RecipeSaveFilter は、台本生成とカット割りを終えた VideoRecipe を保存し、キーフレーム
...
// 二度通しても結果が変わらないため（TestSceneSplitFilterIsIdempotent）、この保存を
// mv_from_keyframe_video_recipe へそのまま渡してもカット割りは保たれます。
type RecipeSaveFilter struct{}

// Name returns the receiver name.
func (RecipeSaveFilter) Name() string { return "recipe_save" }

// Execute validates the recipe and writes it as the job's metadata.
func (RecipeSaveFilter) Execute(ctx context.Context, fc *Context) error {
	...
	return PublishingFilter{}.Execute(ctx, fc)
}
