...
Keyframe images are reused rather than re-baked when a job runs again over an already-planned recipe (e.g. history detail's "generate video → full MV", which goes through `RecipeLoad → SceneSplit → CutKeyframe`). Two halves make that work and both are needed: `SceneSplitFilter` keeps a cut's `KeyframeReference` when the cut re-allocates to exactly one block (a re-split cut drops it — one image can't stand for several cuts that each get their own scene beat), and `CutKeyframeFilter` skips `RunAndSave` when every cut has an absolute keyframe reference. **The second half is a workaround pending a library release**: `CutKeyframeRunner.RunAndSave` in go-veo-orchestrator now skips per cut (a cut with a non-empty `KeyframeReference` is not re-baked, and metadata is written either way), which is strictly better because it also handles partially baked recipes — but ap-mv is pinned to v1.9.3, which predates it. Once the library is tagged and `go.mod` bumped, delete `allCutsHaveKeyframes` and `reuseExistingKeyframes` from `cut_gen.go` and call `RunAndSave` unconditionally. The `SceneSplitFilter` half stays either way: only the planner knows a cut re-allocated 1:1 and may keep its image. The skip still writes `video_music_meta.json` via `Workflows.Publish` — the history list finds jobs by that file, so skipping the write would make a generating job vanish from history for the duration. Relative references (legacy recipes) are not reused: they resolve against their original job's base, not this job's output path. To deliberately re-bake, use `regenerate_cut_keyframe` / `regenerate_section_keyframes`; picking up changed character art or seeds requires one of those, not a video re-run.
